...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Taak Bewerken: {{ $task->name ?? 'Taak #' . $task->id }}</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-8 max-w-md">
        <form action="{{ route('tasks.update', $task->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ $task->name }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ $task->description }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
